...
class InterProceduralInferer():
    ...
    def infer_recursive(self, scope, node):
        callsites = scope.outgoing
        recursive_calls = []
        for callsite in callsites:
            if (callsite.dest.entry.name == callsite.src.entry.name):
                recursive_calls.append(callsite.context) #get all SimpleCallNodes
        from .ParseTreeTransforms import RecursiveTransform,TempTransform
        temp = RecursiveTransform(None, recursive_calls)
        temp(node)
        SimpleAssignmentTypeInferer().infer_types(node.local_scope) 
        temp = TempTransform(None)
        temp(node)
    def infer_types(self, scope):
